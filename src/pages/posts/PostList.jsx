...
const PostList = () => {
        ...
        const [keyword, setKeyword] = useState(""); // keyword는 단순히 검색어 이기 떄문에 배열 XX
...
// 2. 추천 검색어 클릭 시 검색어 입력창에 자동 입력
        function handleSug(sug) {
            setKeyword(sug); // 사용자가 검색어를 클릭해서 input 안에 넣어서 검색할 경우
            setShow(false); // 더이상 추천 리스트를 보여줄 필요가 없기 때문에 추천 리스트 숨기기 설정
        }

        /* 아래와 같은 방법까지 사용할 필요X
        api Service.js 에서 getAllPosts 기능을 불러온 후
        세부 설정을 진행해야할 때 작성
            useEffect(()=>{
...
// 검색 기능 추가
        ...
        onBlur = 입력 필드에서 포커스가 벗어날 때 실행되는 이벤트
         ...
         onMouseDown = 마우스를 클릭하는 순간 실행되는 이벤트
                       사용자가 추천 검색어를 클릭할 경우 추천 검색어 리스트가 onBlur 로 인해 사라지고
                       마우스 클릭을 누르는 순간 제안한 리스트 중에서 사용자가 선택한 제안을 키워드 내부로 들어갈 수 있도록 설정

         onMouseDown={() => handleSug(sug)}
        */

        return (
            <div className="postsearch-container">
                <h2>게시물 목록</h2>
